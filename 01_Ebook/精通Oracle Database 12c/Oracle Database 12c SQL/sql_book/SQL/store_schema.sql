...
--   4. Populates填入 the database tables with sample data

-- attempt 试图 to drop the store user (this will generate an error
-- if the store user does not yet exist; do not worry about this
-- error); this statement 声明 is included so that you do not have
-- to manually手动地 run DROP before recreating the schema模式
DROP USER store CASCADE;

-- create the store存储 user 
-- IDENTIFIED经鉴定的 by后面是密码
...
-- give the store user a quota of 10M on the users tablespace
-- 为store 用户在users表空间分配10M空间
-- SELECT t.* from database_properties t WHERE  t.PROPERTY_NAME='DEFAULT_PERMANENT_TABLESPACE';--t.property_value
-- property_value是users 不用改 是其他需要更改；
...
CREATE TABLE employees3 (
  employee_id INTEGER CONSTRAINT employees3_pk PRIMARY KEY,
  division_id CHAR(3)
    CONSTRAINT employees3_fk_divisions
    REFERENCES divisions(division_id),
  job_id CHAR(3) REFERENCES jobs(job_id),
  first_name VARCHAR2(10) NOT NULL,
  last_name VARCHAR2(10) NOT NULL,
...
-- insert sample data into employees3 table
...
-- insert sample data into all_sales2 table
INSERT INTO all_sales2 (
...
-- insert sample data into all_sales3 table
INSERT INTO all_sales3 (
  product_id, total_amount, sale_date
) VALUES (
  1, 1000, '01-JUN-2011'
);
